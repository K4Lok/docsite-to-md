...
These numbers were captured with the release binary, `--max-pages 100`, `--concurrency 8`, and live network requests. They are a snapshot, not a guarantee: live sites vary with network latency, page size, rate limits, and site markup. Re-run the table with `bash scripts/live-benchmark.sh`.
